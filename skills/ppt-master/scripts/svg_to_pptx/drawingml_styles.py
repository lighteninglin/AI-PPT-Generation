...
def build_gradient_fill(
    ...
    if tag == 'linearGradient':
        ...
        # DrawingML: 0°=up(north), clockwise; SVG atan2: 0°=right(east), CCW
        dml_angle_deg = (90 - angle_deg) % 360
        dml_angle = int(dml_angle_deg * ANGLE_UNIT)
...
<a:lin ang="{dml_angle}" scaled="1"/>
        ...
        # Map SVG cx/cy/r to DrawingML fillToRect (all in 1/1000-percent)
        try:
            cx = float(grad_elem.get('cx', '0.5'))
            cy = float(grad_elem.get('cy', '0.5'))
            r = float(grad_elem.get('r', '0.5'))
            # Clamp to [0, 1]
            ...
            cx, cy, r = 0.5, 0.5, 0.5
        # fillToRect defines the bounding box of the gradient circle
        # l = left edge, t = top edge, r = right edge, b = bottom edge (from center)
        half = int(r * 100000)
        center_x = int(cx * 100000)
        center_y = int(cy * 100000)
        l = center_x - half
        t = center_y - half
        ri = 100000 - center_x - half
        b = 100000 - center_y - half

        return f'''<a:gradFill>
<a:gsLst>{gs_list}</a:gsLst>
<a:path path="circle">
<a:fillToRect l="{l}" t="{t}" r="{ri}" b="{b}"/>
</a:path>
</a:gradFill>'''
