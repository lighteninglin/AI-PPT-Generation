...
def _build_text_runs(
    ...
    for child in elem:
        ...
        if child_tag == 'tspan':
            ...
            # SVG <br/> inside <text> → newline (DrawingML paragraph break)
            runs.append({**parent_attrs, 'text': '\n'})
            if child.tail:
                t = _normalize_text(child.tail, preserve_space=preserve_space)
                if t:
                    runs.append({**parent_attrs, 'text': t})
...
def convert_text(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    # Split runs into paragraph groups at '\n' (from <br/> elements)
    para_groups: list[list[dict[str, Any]]] = []
    ...
    para_groups.append(current_group)
    # Filter out empty groups (consecutive <br/>)
    para_groups = [g for g in para_groups if g]
    num_lines = len(para_groups)

    # Estimate text dimensions
    full_text_no_nl = full_text.replace('\n', '')
    text_width = estimate_text_width(full_text_no_nl, font_size, font_weight) * 1.15
    text_height = font_size * 1.5 * num_lines
    ...
    # Build paragraph XML: one <a:p> per paragraph group (split by <br/>)
    paragraphs_xml: list[str] = []
    for para_runs in para_groups:
        para_runs_xml = '\n'.join(
            _build_run_xml(r, fonts, ctx, text_effect_xml) for r in para_runs
        )
